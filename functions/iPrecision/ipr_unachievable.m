function [p, r, au] = ipr_unachievable(pi1, pi2)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Calculates the unachievable area of integrated Precision-Recall space for
% a specific dataset skew range, pi1 and pi2, and the area under its curve.
% 
...
if use_integral
    ...
    p = zeros(size(r));
    for r_ind = 1:numel(r)
        f = @(pi) (pi .* r(r_ind)) ./ ((pi .* r(r_ind)) + (1 - pi));
    ...
    p = zeros(size(r));
    for r_ind = 1:numel(r)
        p(r_ind) = sum((pi .* r(r_ind)) ./ ((pi .* r(r_ind)) + (1 - pi)));
    end
    p = p / numel(pi); % Eq. (19)
...
% Calculate the area under the curve using Theorem 9
au = (1 / (pi2 - pi1)) * ((-2 * pi1) + (2 * pi2) ...
     + (-1 + pi1) * log(1 - pi1) - ((-1 + pi2) * log(1 - pi2)) ...
     + dilog(pi1) - dilog(pi2)); 

% OR Using trapezoidal integration of the P-R curve, Eq. (20)
%au = auc(p,r);
