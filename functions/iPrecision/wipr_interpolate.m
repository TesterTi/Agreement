function [ps, rs] = wipr_interpolate(TP_a, TP_b, FP_a, FP_b, tn, fn, pi1, pi2, w_function)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Interpolates between two points in weighted integrated P-R space.
%
...
% 
%
% Detailed in Section 3.3.2 and Appendix C, Section 1 of the paper:
%
...
if use_integral
   ...
   ps = zeros(1,numel(tp));
   for i = 1:numel(tp)
      p = @(pi) ((w_function(pi)./pi_m) .* pi * tp(i)) ./ ((pi * tp(i)) + ((1 - pi) .* phi .* (FP_a + s*i)));
   ...
   ps = 0;
   for x = 1:numel(pi)
       ps = ps + (((w_function(pi)./pi_m) .* pi(x) .* tp) ./ ((pi(x) * tp) + ((1 - pi(x)) * phi * (FP_a + s*[1:numel(tp)]))));
   end
   ps = ps/numel(pi);
