function [p, r, au, threshold_values] = pr_curve(response, gt, interpolate)
...
% Run through threshold values to evaluate the P-R curve
...
for i = 1:numel(threshold_values)
    ...
    [~, ~, p(i), r(i), tp(i), fp(i), ~, fn(i)] = pr_point(detection, gt, [neg_label pos_label]);
...
if interpolate
    ...
    for j = 1:number_of_data_points-1
        ...
        [ps1, rs1] = pr_interpolate(tp(j+1), tp(j), fp(j+1), fp(j), fn(j));
